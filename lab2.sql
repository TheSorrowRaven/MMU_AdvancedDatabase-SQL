...
SELECT VARCHAR_FORMAT(ENGAGEMENT_DATE, 'Month') AS month_name, COUNT(ENGAGEMENT_ID)
FROM ENGAGEMENT
WHERE YEAR(ENGAGEMENT_DATE) = '2015'
GROUP BY VARCHAR_FORMAT(ENGAGEMENT_DATE, 'Month');

SELECT TO_CHAR(ENGAGEMENT_DATE, 'Month') AS month_name, COUNT(ENGAGEMENT_ID)
FROM ENGAGEMENT
WHERE EXTRACT(YEAR FROM ENGAGEMENT_DATE) = 2015
GROUP BY TO_CHAR(ENGAGEMENT_DATE, 'Month');

SELECT EQUIPMENT_DESC, CHARGE_PER_DAY FROM EQUIPMENT
WHERE EQUIPMENT_DESC LIKE '%Canopy';

SELECT C.CLIENT_NAME, COUNT(E.CLIENT_ID), SUM(E.FEE) FROM CLIENT C
JOIN ENGAGEMENT E ON E.CLIENT_ID = C.CLIENT_ID
GROUP BY C.CLIENT_NAME;

SELECT * FROM EQUIPMENT E
WHERE NOT EXISTS (
	SELECT * FROM EQUIPMENT_USE U
	WHERE U.EQUIPMENT_NO = E.EQUIPMENT_NO
);

SELECT E.EVENT_ID FROM EVENT E
JOIN ENGAGEMENT G ON E.EVENT_ID = G.EVENT_ID
GROUP BY E.EVENT_ID
